...
class CornersProblem(search.SearchProblem):
    ...
    def isGoalState(self, state: Any):
        ...
        #if this is the first time we are visiting a corner
        if coordinates in self.corners and coordinates not in visited_corners:
            #add this corner to the visited list
            visited_corners.append(coordinates) 
        #count the number of corners that we have visited
        for corner in visited_corners:  
            counter += 1
        #if we have visited all the corners return True else return False
